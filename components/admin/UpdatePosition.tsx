...
const UpdatePosition = (props: IProp) => {
  ...
  const getArrive = async () => {
    try {
      const res = await axios.get(
        "https://qlcv-server.herokuapp.com/api/positions/" + props.id,
        {
          headers: {
            Authorization: `Bearer ${loginReducer.token}`,
          },
  ...
  return arriveDetail ? (
    <View style={styles.container}>
      <Text style={styles.title}>Cập nhật chức vụ</Text>
      <View style={styles.infoContainer}>
        <Controller
          ...
          name="tencv"
          rules={{ required: true }}
          defaultValue={arriveDetail.tencv || ""}
        />
        <Controller
          control={control}
          render={({ field: { onChange, onBlur, value } }) => (
        ...
        <Controller
          ...
          name="quyenduyet"
          rules={{ required: true }}
          defaultValue={arriveDetail.quyenduyet || ""}
        />
      </View>
      {loading ? (
        <Button
