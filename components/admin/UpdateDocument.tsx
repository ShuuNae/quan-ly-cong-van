...
const Updatedocument = (props: IProp) => {
  ...
  const getArrive = async () => {
    try {
      const res = await axios.get(
        "https://qlcv-server.herokuapp.com/api/documentTypes/" + props.id,
        {
          headers: {
            Authorization: `Bearer ${loginReducer.token}`,
          },
  ...
  return arriveDetail ? (
    <View style={styles.container}>
      <Text style={styles.title}>Cập nhật loại văn bản</Text>
      <View style={styles.infoContainer}>
        <Controller
          ...
          name="tenlvb"
          rules={{ required: true }}
          defaultValue={arriveDetail.tenlvb || 0}
        />
        <Controller
          control={control}
          render={({ field: { onChange, onBlur, value } }) => (
          ...
          name="ghichu"
          rules={{ required: true }}
          defaultValue={arriveDetail.ghichu || 0}
        />
      </View>
      {loading ? (
        <Button
